...
class AV_Dataset(Dataset):
    def __init__(self, video_path, csv_file, transform=None, mode="video"):
        super().__init__()
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == "video":
            img_frames = img_frames.permute(0, 3, 1,
                                            2)  # (N, H, W, C) -> (N, C, H, W) / N: numero de frames, C: numero de channels
            ...
            if self.transform == transform_toMelSpec_db:
                ...
                audio_batch = torch.stack(
                    [self.transform(audio_frames[i], 48000, 16000) for i in range(0, len(audio_frames))])
